...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-41 CLOSED — session/token lifecycle done**. Created `spec/31-app/05-conventions/11-session-token-lifecycle.md` v1.0.0: 5 token kinds (WPSession / AccessToken 15m-idle 8h-abs / RefreshToken 7d-idle 30d-abs / SSEToken 30s / CsrfToken), mandatory RefreshToken rotation with family-revoke on reuse detection, `TokenRevocationList` with 60s propagation deadline matching A-40, SSE one-shot ticket flow, 30-day HMAC secret rotation with 24h overlap, 10-session-per-user cap with oldest-eviction, 9 new audit codes for combined v1.2.0 backfill batch, hygiene gate **G-25** (token-lifecycle coverage forbids `localStorage`/`sessionStorage` for tokens), 14 ATs `AT-TOKEN-01..14`. Prior: A-40 (role-escalation, G-24), A-39 (audit-log, G-23), A-37 (error-code SSOT, G-22), A-36 (rate-limiting). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
