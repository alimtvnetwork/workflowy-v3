...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-30 CLOSED** — created `spec/31-app/05-conventions/02-ci-quality-gates.md`: catalogue of all 14 gates (G-01..G-16) with exit-code contract, hard-vs-generator semantics, single CI entry point (`bun run spec:check`), procedure for adding new gates, and 8 acceptance tests (`AT-CIGATE-01..08`). Reserved G-17/G-18 for the hotkey + cycle-SQL drift checks promised by A-19/A-38. Wired into conventions overview. Prior: A-38 closed (mirror cycle detection algo), A-28 closed (mirror-as-ItemType drift sweep), A-19 closed (hotkey table SSOT), A-03 closed (PowerShell overview split). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
