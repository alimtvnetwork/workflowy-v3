...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-33 CLOSED** — created `spec/31-app/05-conventions/05-precommit-hook-contract.md`: SSOT for installer (`scripts/install-git-hooks.mjs`) + hook body (`scripts/git-hooks/pre-commit`). 7-row installer contract, 8-step hook recipe, POSIX-sh portability rules, forbidden-construct list. Identifies same single-entry-point violation in shipped hook (uses `node` not `bun run spec:check`); reserves G-20 hygiene gate (A-34). 10 acceptance tests `AT-PRECOMMIT-01..10`. Prior: A-32 (G-19 workflow gate), A-31 (workflow contract), A-30 (CI gate runner), A-38 (mirror cycle algo). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
