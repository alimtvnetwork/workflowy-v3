...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-37 CLOSED — error-code SSOT done**. Created `spec/03-error-manage/02-error-architecture/05-response-envelope/05-error-code-catalogue.md` v1.0.0: catalogued all **74** existing `ERR_*` codes across 9 domains (auth, validation, items/tree, policy, config, I/O, installer, git, sentinel) with 6-attribute schema (Code/HttpStatus/Severity/Retryable/Tier/MessageKey), 4-tier severity model, 3-mode retry contract, append-only deprecation procedure (6 Go-toolchain codes flagged ⚠️ for removal in v0.40.0), G-22 catalogue-drift gate proposed. Prior: A-36 (rate-limiting), A-35 (G-21), A-30..A-35 (CI cluster). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
