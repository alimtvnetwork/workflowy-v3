...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-34 CLOSED** — created `spec/31-app/05-conventions/06-g20-precommit-contract-gate.md`: algorithm SSOT for the G-20 hygiene gate detecting drift between hook/installer files and A-33's contract. 6 installer rules (INST-01..06) + 10 hook rules (HOOK-01..10), 4 match-function semantics, 10 acceptance tests `AT-G20-01..10`. Sibling to G-19 (workflow gate). Together G-19 + G-20 enforce that every CI artefact stays in sync with its contract spec. Prior: A-33 (pre-commit contract), A-32 (G-19 algorithm), A-31 (workflow contract), A-30 (CI gate runner SSOT). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
