...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-31 CLOSED** — created `spec/31-app/05-conventions/03-github-actions-workflow.md`: contract for required `spec-hygiene.yml` workflow (triggers, job fields, 5-step recipe, 4 drift-detection targets) plus reserved `code-checks.yml` (Phase 1) and `release.yml` (Phase 5). Identifies gap that current workflow uses `node` directly instead of `bun run spec:check` — A-32 will add G-19 hygiene gate to enforce. 8 acceptance tests `AT-WORKFLOW-01..08`. Wired into conventions overview. Prior: A-30 (CI gate runner SSOT), A-38 (mirror cycle algo), A-28 (mirror-as-ItemType drift sweep), A-19 (hotkey table). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
