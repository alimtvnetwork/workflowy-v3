...
> **Updated:** 2026-04-26 (UTC+8) · **Status:** 🎉 **A-39 CLOSED — audit-log policy done**. Created `spec/31-app/05-conventions/09-audit-log-policy.md` v1.0.0: 22-action taxonomy across 7 categories (AUTH/AUTHZ/DATA/SHARING/ADMIN/POLICY/SYSTEM), 365/90-day retention floors with hard ceilings, SHA-256 + daily-rotated-salt PII scrubbing (14-day correlation window), append-only hash-chain integrity (`IntegrityHash = SHA256(prev || canonicalJson)`), 3-endpoint query API (`EP-AUDIT-LIST/EXPORT/VERIFY`) with `audit.read/export/admin` roles, coalescing + sampling for volume control, gate **G-23** (audit-coverage drift), 15 acceptance tests `AT-AUDIT-01..15`. Three streams now formally separated: activity-feed (user) vs operational-log (operator) vs audit-log (compliance). Prior: A-37 (error-code SSOT, G-22), A-36 (rate-limiting), A-30..A-35 (CI cluster). Hygiene 17/18; only **A-01** remains — `src/types/index.ts` `mirror` → `dashboard`, gated by `mem://constraints/spec-only-mode`. **Say `exit spec-only` to apply that fix and start P1.1 Bootstrap.**
