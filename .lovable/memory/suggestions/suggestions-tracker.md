...
> **Updated:** 2026-04-25 (UTC+8) — A-24 closed (13th batch of leaf AT files curated; running totals: 66 AT files curated → ~837 testable criteria authored)
...
- **Closed:** 2026-04-25 (UTC+8)
- **Result:** Curated `15-wp-plugin-how-to/08-wordpress-integration-patterns/` (16, AT-WORDPRESSINTEGRATIONPATTERNS-01..16 — admin pages registered only on admin_menu hook, custom per-plugin capability not manage_options as privilege-escalation guard, mandatory nonce+capability on every wp_ajax_* handler, EnvelopeBuilder-only AJAX responses, cron registered on activation+unregistered on deactivation, SafeExecute-wrapped cron callbacks with start/finish/error logs, MIME+extension+size validation on uploads, plugin-scoped uploads subdir, versioned idempotent migrations via single MigrationRunner, opt-in seeders forbidden on production activation, namespaced transient keys with explicit TTL, read-through cache pattern, HttpClient+HttpConfigType-only outbound HTTP forbidding curl_exec/file_get_contents on URLs, mandatory requestId+host+status+elapsed-ms log fields, fixed filter-before-actions hook order, enum inventory mirrored in spec/20-enums-index.md), `15-wp-plugin-how-to/13-admin-ui-patterns/` (15, AT-ADMINUIPATTERNS-01..15 — canonical 3-region admin layout header+filters+content, max one Primary button per actions bar, URL-roundtripped filter state with no JS-only filters, removable filter chips with single Clear-all, <Table> primitive only with no raw <table>, mandatory 4 table states loading/empty/filtered-empty/error, semantic badge colour mapping Code-Red on off-spec, modal anatomy header+scrollable body+Primary-right/Cancel-left footer, <Notice> primitive only with no raw notice div markup, persistent notices require remediation CTA, content-shape skeletons not generic spinners, empty states with illustration+headline+text+CTA, determinate vs indeterminate progress contract, pagination jump controls when >5 pages, mandatory accessibility rules focus-rings+tab-order+keyboard-shortcuts), `15-wp-plugin-how-to/16-error-handling-extraction/` (16, AT-ERRORHANDLINGEXTRACTION-01..16 — ErrorType enum classification with no ad-hoc string codes, two-tier capture boundary+shutdown handler, paginated error-log retrieval capped at 100 with errorType/severity/dateRange/requestId filters, mandatory admin capability on retrieval as OWASP guard, sessions group by requestId+userId+5-min window, sessions store stacktrace path not inline, vanilla ES2020+ admin error JS, flash banner timing 5s success/10s warning/sticky error, auto-refresh ≤30s pausing on hidden tab, SafeExecute as sole try/catch with mandatory log+requestId+ErrorType+stacktrace, seedable-config-driven notification thresholds with no hardcoding, ErrorType→HTTP+ResponseKeyType deterministic mapping in §10 SSOT, AdminErrorAjaxTrait as sole error-UI AJAX source, sessions-not-rows admin template with stacktrace-detail modal, error_sessions schema PascalCase columns + RequestId+LastSeenAt indexes, §14 merge-gate checklist), `15-wp-plugin-how-to/19-micro-orm-and-root-db/` (14, AT-MICROORMANDROOTDB-01..14 — fluent immutable ORM shell returning new instances, parameterized-only WHERE Code-Red on string concat, fluent get/first/count/exists/pluck with raw() only via Db::raw value object, mandatory explicit WHERE on update/delete throwing on unconstrained mutation, runnable §05 examples via CI doc-test, ORM-wrapped $wpdb only with no direct $wpdb in business code, dynamic $wpdb->prefix with no hardcoded wp_ multi-site Code-Red, §07 patterns merge-gate, PascalCase column casing per cross-language SSOT, readonly DTO hydration with no array-typed row returns, typed-query column-mismatch throws, atomic write-then-rename file cache against partial-write corruption, mandatory version+TTL header on cache files, single Cache::invalidate API with no scattered unlink), `15-wp-plugin-how-to/20-end-to-end-walkthrough/` (18, AT-ENDTOENDWALKTHROUGH-01..18 — single-feature scope with 1-paragraph user story, canonical includes/templates/assets/tests/data layout, header-only bootstrap + single require, PSR-4 autoloader before all code, label/values/tryFromOrThrow on every walkthrough enum, FileLogger plugin-scoped + size+age rotation + canonical 5-field entries, stateless single-responsibility helper traits, fixed plugin.php init order constants→container→services→helpers→hooks, endpoints.json-driven route loop, versioned idempotent migrations via MigrationRunner, thin controllers + behaviour traits, admin settings page in 3-region admin layout, settings via SSOT registry not raw add_settings_section, tests covering enum+route happy-path+route error-path+migration up/down+settings save+SafeExecute fallback, idempotent uninstall removing tables+options+transients+files+cron, phase-coverage matrix mapping each section to parent spec, §17 final-checklist merge gate, CI doc-test for snippet parseability). Total: 79 testable criteria. All 18 hygiene checks pass. Stub count: 35 → 30. Version 0.23.0.
