...
### A-27 — Polish #4: per-phase pre-flight implementation checklist (SPEC-ONLY → BUILD handoff gate) → ✅ closed
- **Closed:** 2026-04-26 (UTC+8)
- **Result:** Created [`spec/31-app/04-roadmap/03-implementation-checklist.md`](../../../spec/31-app/04-roadmap/03-implementation-checklist.md) — a per-phase pre-flight checklist the next AI session walks before writing the first line of code. Covers Phase 0 (SPEC-ONLY exit gate, 5 checks), P1.1 Bootstrap (7), P1.2 Tokens (5), P1.3 App Shell (5), P1.4 Auth (4), P1.5 Outliner+CRUD (7), P2 Rich Editing (5), P3 Navigation (5), P4 Collaboration (7), P5 Polish (7), plus 7 cross-cutting end-of-phase gates. Total: **64 atomic pre-flight checks** with source-of-truth links to memory rules, prior audit findings (A-06/A-07/A-08 already done; F-04 / Plan 07 / Plan 08 referenced), and acceptance criteria (`AT-APP-29..32`, `AT-APP-54`, `AT-DOCSVIEWERUI-06/08`, `AT-USERSCOPEDISOLATION-12/13`). Updated `spec/31-app/04-roadmap/00-overview.md` → v1.1.0 (Files table + Related list extended; AUTO-TOC sentinels left for the regen script). Updated `.lovable/plans/00-active.md` header. Closes the SPEC-ONLY → BUILD transition contract.

### A-28 — Polish #5: end-to-end audit (option-b) + close 3 spec-only-safe findings → ✅ closed
- **Closed:** 2026-04-26 (UTC+8)
- **Result:** Generated [`.lovable/reports/02-ai-readiness-report-post-a27.md`](../../../.lovable/reports/02-ai-readiness-report-post-a27.md) — a full end-to-end audit of the 1270-file spec corpus + 1720 unique `AT-*` IDs. Hygiene runner score: **17/18 pass** (94/100). Surfaced 4 findings: **F-AUD27-01** `ItemType` enum drift between spec SSOT and `src/types/index.ts` (`mirror` should be `dashboard` per Round-3 AUDIT-03; 3-line TS edit blocked by spec-only mode), **F-AUD27-02** 2 phantom AT-stubs auto-generated in already-folded folders, **F-AUD27-03** missing `00-overview.md` in `spec/03-error-manage/03-error-code-registry/01-registry/`, **F-AUD27-04** acceptance-coverage-runner warnings (resolved by F-AUD27-02 fix). **Wave-1 fixes (spec-only-safe):** Replaced `spec/31-app/02-workflows/97-acceptance-criteria.md` (v1.0.0 dispatch index → canonical `AT-APP-43..57`), replaced `spec/31-app/04-roadmap/97-acceptance-criteria.md` (v1.0.0 dispatch index → checklist + canonical app rollup, no `AT-ROADMAP-NN` namespace), created `spec/03-error-manage/03-error-code-registry/01-registry/00-overview.md` (v1.0.0 wraps existing `01-overview.md` to satisfy required-files invariant), replaced same folder's auto-stubbed `97-acceptance-criteria.md` (v1.0.0 dispatch → parent `AT-ERRORCODEREGISTRY-NN`). Re-run hygiene: 17/18 pass (only F-AUD27-01 remains, blocked by spec-only mode). Updated `.lovable/plans/00-active.md` header. Closes 3 of 4 audit findings entirely within spec-only mode.

### A-29 — Polish #6 (option-a): promote `spec/18-spec-issues/97-acceptance-criteria.md` from scaffold → dispatch index → ✅ closed
- **Closed:** 2026-04-26 (UTC+8)
- **Result:** Bumped `spec/18-spec-issues/97-acceptance-criteria.md` from v1.0.1 → v1.1.0. Replaced "Status: Scaffold — meta/tracking folder" with "Status: Dispatch index — meta/tracking folder; per-audit ACs live inline in each numbered file". Content was already curated (8-row Coverage Map covering audits + gates, all 3 criteria checked); only the misleading "Scaffold" label triggered the spec-hygiene runner's stub heuristic. Re-run hygiene: still 17/18 pass (only F-AUD27-01 — TS `ItemType` drift — remains, still blocked by spec-only mode). **All optional spec-only polish is now exhausted.** The only remaining work in the entire spec corpus is F-AUD27-01, which requires `exit spec-only` to apply.
