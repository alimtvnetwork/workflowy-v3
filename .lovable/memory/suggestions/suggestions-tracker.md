...
> **Updated:** 2026-04-25 (UTC+8) — A-22 closed (11th batch of leaf AT files curated; running totals: 56 AT files curated → ~685 testable criteria authored)
...
- **Closed:** 2026-04-25 (UTC+8)
- **Result:** Curated `15-wp-plugin-how-to/02-enums-and-coding-style/` (13 criteria, AT-ENUMSANDCODINGSTYLE-01..13 — PHP 8.1+ backed enums only, one-enum-per-file with includes/Enums/<Name>.php layout, PascalCase backing values, match()-based metadata dispatch with no nested if/switch, mandatory label()/values()/tryFromOrThrow() trio, SelfUpdateStatus exact-cases-list with isTerminal() method, ActionType as audit-log SSOT, strict === comparison, registry in spec/20-enums-index.md, consistency report regen on every enum change), `15-wp-plugin-how-to/04-logging-and-error-handling/` (16, AT-LOGGINGANDERRORHANDLING-01..16 — two-tier operational+debug log split, RISEUP_DEBUG constant in wp-config.php, plugin-scoped log path with no debug.log fallback, canonical 5-field entry format with requestId propagation, sibling stacktrace file, dual size+age rotation thresholds via seedable-config, transport JSON envelope distinct from on-disk, 60s dedup window keyed (level,source,message), Code-Red on swallowing catches, SafeExecute as sole try/catch wrapper with mandatory $context, Helpers\\Responses::error wrapper for WP_Error, ErrorLogHelper-only file logging, wp_date timezone-aware dates, mandatory shutdown-handler), `15-wp-plugin-how-to/14-rest-api-conventions/` (16, AT-RESTAPICONVENTIONS-01..16 — single canonical namespace with v<integer> versioning, kebab-case route paths, GET/POST/PATCH/PUT/DELETE semantic mapping with no PUT-as-create, EndpointType enum SSOT, single endpoints.json-driven register_rest_route loop, offset+limit pagination capped at 100 with total/page/perPage/totalPages envelope, filter[<field>] syntax, RequestFieldType enum for arg validation, ResponseKeyType-only response keys Code-Red, one-controller-per-resource, endpoints.json as route metadata SSOT, 5 standard endpoints Ping/Whoami/Health/Version/Diagnostics, dynamic-segment regex contract, auto-generated OpenAPI 3.1 with no manual edits), `15-wp-plugin-how-to/15-settings-architecture/` (14, AT-SETTINGSARCHITECTURE-01..14 — single options row per group with riseup_settings_<slug> naming, SSOT registry forbidding ad-hoc add_settings_section, kebab-case group slugs, mandatory default-at-registry-time, RequestFieldType-typed sanitizers with sanitize_text_field-as-catch-all forbidden, WP_Error field-level error map, fixed 3-column page layout, restricted field-types whitelist, toggle-switch-not-checkbox for booleans, metadata-driven conditional display, selection-cards/sliders for finite-enum/bounded-numeric, no-inline-onclick action buttons, atomic dual-save with rollback Code-Red, anti-patterns ban god-options-row + raw update_option), `02-coding-guidelines/consolidated-review-guide/` (15, AT-CONSOLIDATEDREVIEWGUIDE-01..15 — mandatory 3-step PR workflow with retro for every Code-Red violation, ≤15 logical lines + ≤200 file lines + ≤3 params + ≤2-tuple returns, naming matrix enforcement, positive boolean prefixes with no plurals/negatives, zero nested if Code-Red, magic-number ban, any/interface{}/mixed forbidden in business code, apperror envelope with no fmt.Errorf/new Error/WP_Error::add direct calls, parallel async via Promise.all/errgroup/Guzzle\\Pool, PascalCase DB identifiers + parameterized SQL Code-Red on concat, structured logs with stacktrace + PII redaction, OWASP-Top-10 + cache-invalidation reviewer pass). Total: 74 testable criteria. All 18 hygiene checks pass. Stub count: 45 → 40. Version 0.21.0.
