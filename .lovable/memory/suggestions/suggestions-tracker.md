...
> **Updated:** 2026-04-25 (UTC+8) — A-21 closed (10th batch of leaf AT files curated; running totals: 51 AT files curated → ~611 testable criteria authored)
...
- **Closed:** 2026-04-25 (UTC+8)
- **Result:** Curated `02-coding-guidelines/01-cross-language/27-types-folder-convention/` (14 criteria, AT-TYPESFOLDERCONVENTION-01..14 — dedicated `types/` subfolder per domain, max 1-level nesting, no upward imports, one-definition-per-file rule, named generic aliases, immutable type values, 5 canonical common types ContentType/HttpMethod/HttpStatus/SortDirection/Environment, byte-identical semantics across Go/TS/PHP), `02-coding-guidelines/04-php/09-response-key-type-inventory/` (13, AT-RESPONSEKEYTYPEINVENTORY-01..13 — exactly 176 PascalCase cases as of v2.1.0, byte-identical PHP/Go/TS mirror parity, 11 topic files <200 lines each with zero gap/overlap, mandatory enum methods, no string-literal response keys, deprecation policy with 1-minor-version grace), `02-coding-guidelines/03-golang/01-enum-specification/02-required-methods/` (14, AT-REQUIREDMETHODS-01..14 — exactly 11 mandatory methods String/Label/Is{Value}/All/ByIndex/Parse/IsValid/IsOther/IsAnyOf/MarshalJSON/UnmarshalJSON, String() as SSOT with Label() delegation, apperror on Parse failure, IsAnyOf with empty-set returns false, optional Values() in sync with All(), domain methods in sibling file, PascalCase variantLabels with protocol-driven exceptions, internal/enums/<domain>/ folder layout, registration in spec/20-enums-index.md), `05-split-db-architecture/01-fundamentals/` (15, AT-SPLITDBFUNDAMENTALS-01..15 — Root/Domain/History/Cache DB SSOT terminology, 2-4 layer hierarchy cap, mandatory WAL mode, per-DB connection pools, online-backup API for incremental backup with WAL-checkpoint, DbManager interface gateway, idempotent GetOrCreateDb concurrency, canonical `<root>/<type>/<scope-slug>/<name>.db` paths, deterministic slug generation, lifecycle ordering, selective ZIP export with manifest, structured DB-operation logs), `06-seedable-config-architecture/01-fundamentals/` (15, AT-SEEDABLECONFIGFUNDAMENTALS-01..15 — version-driven merge-vs-skip with user-override preservation, mandatory 3-artifact set seed.json+schema.json+CHANGELOG.md, build-time schema validation gate, ConfigMeta+Setting+SettingHistory PascalCase tables with mandatory audit rows, typed SettingValue union with no any/interface{}, ConfigService gateway with idempotent SeedWithVersionCheck, semver bump rules major/minor/patch, mandatory version-badge+new-settings UI when settings UI ships, theme CSS-variable contract with no raw hex, no-hardcoded-arrays policy). Total: 71 testable criteria. All 18 hygiene checks pass. Stub count: 50 → 45. Version 0.20.0.
