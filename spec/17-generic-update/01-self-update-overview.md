...
- No interactive prompts during the build/deploy phase (only during initial repo resolution when `git remote get-url origin` returns non-zero exit code).
...
| App Spec | Covers |
|----------|--------|
| `02-powershell-build-deploy.md` *(canonical PowerShell build/deploy doc pending — see spec/10-powershell-integration/)* | Full build pipeline, config, deploy, and run patterns |
| [03-self-update-mechanism.md](../13-cicd-pipeline-workflows/06-self-update-mechanism.md) | Handoff, skip-if-current, version verification, rollback |
| [09-build-deploy.md](../16-generic-cli/11-build-deploy.md) | gitmap-specific build, deploy, and `-R` flag behavior |
