...
- **SSOT for enum:** [`22-status-legend.md`](./22-status-legend.md) §1 (9 values: `DRAFT`, `REVIEW`, `CANONICAL`, `COMPANION`, `DISPATCH`, `DEFERRED`, `DEPRECATED`, `REDIRECT`, `ARCHIVED`).
...
- **Failure mode (hard-fail mode):** CI emits `<file>:<line>: non-canonical status '<value>' — see spec/01-spec-authoring-guide/22-status-legend.md §2 for canonical mapping` and exits non-zero.
...
- **Canonical value shapes:**
  ...
  - **AI Confidence / AI Implementability** → one of the 5 canonical tokens, exact case, no decoration: `Very High`, `High`, `Medium`, `Low`, `Very Low`. Legacy `Production-Ready` mapped to `Very High`. Trailing emoji (`✅`, `🟢`) FORBIDDEN. Prose rationale MUST live outside the cell (e.g. as a `> **AI Confidence rationale:** …` blockquote below the table).
  - **Ambiguity** → one of the 5 canonical tokens, exact case, no decoration: `None`, `Low`, `Medium`, `High`, `Very High`. Trailing emoji FORBIDDEN.
- **Rules (all hard-fail from 2026-04-29 post-sweep):**
  1. **Health Score canonical shape** — within **every** `^(##|###)\s+Scoring\b` block in the file, the Health Score / Overall / Total row's value cell MUST match `^\d{1,3}%\s\([A-F][+\-]?\)$` after trimming whitespace.
  2. **AI Confidence canonical token** — value cell MUST be one of the 5 tokens above, exact case, no trailing emoji or parenthetical.
  3. **Ambiguity canonical token** — value cell MUST be one of the 5 tokens above, exact case, no trailing emoji.
  4. **Single Scoring section per file** — every top-level overview MUST contain exactly one `^(##|###)\s+Scoring\b` heading. Per-feature criterion grids MUST use a different heading (e.g. `## Quality Breakdown`, `## Per-Feature Scoring`).
- **Scope:** **all** `^(##|###)\s+Scoring\b` blocks across all 25 top-level `spec/[0-9][0-9]-*/00-overview.md`. Sub-overview tier remains out of scope. The previous "first block per file" carve-out was retired 2026-04-29 after the dedupe sweep brought all overviews to exactly one Scoring heading.
- **Baseline (2026-04-29, post-dedupe sweep):** **25 of 25 fully compliant.** Two sweeps: (a) value-format normalized 10 cells across 5 files (`03`/`05`/`06`/`07`/`12`); (b) dedupe renamed `08`'s second `## Scoring` → `## Quality Breakdown` and removed stale boilerplate dupe blocks in `34`/`36` (12 + 14 lines respectively, including duplicate `## Confidence` heading whose rationale was preserved as a `> **Confidence rationale:**` blockquote below the canonical Scoring). Verified by `awk` linter walking **all** Scoring blocks — zero violations. Gate **hard-fail rules 1+2+3+4 from day 1**.
