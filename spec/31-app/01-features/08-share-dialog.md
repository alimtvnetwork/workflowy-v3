...
- **N/A justification:** Per-share permissions are enums (`Role`), not booleans; `IsPublicEnabled` is a per-share row flag, not a global setting.
...
|--------|------|-----------|---------------------------|
| `POST` | `/wp-json/workflowy/v1/shares` | `CreateShare (per-item, per-role)` | IdempotencyKey |
| `PATCH` | `/wp-json/workflowy/v1/shares/{ShareId}` | `UpdateShareRole` | IdempotencyKey |
| `DELETE` | `/wp-json/workflowy/v1/shares/{ShareId}` | `RevokeShare` | IdempotencyKey |
| `POST` | `/wp-json/workflowy/v1/shares/{ShareId}/public-toggle` | `TogglePublicEnabled` | IdempotencyKey |
...
- **Tables touched:** cross-db: root.share_invites + app.shares
- **Error boundary on failure:** `PanelBoundary`
