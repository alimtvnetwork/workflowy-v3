...
- **ADR-0017 — Named Error Boundaries:** This feature renders inside **`SearchBoundary`**. A single top-level boundary is **forbidden**. Loader/action errors surface via the matching named boundary; uncaught render errors escalate to `AppErrorBoundary`. See `spec/30-architecture/adr/0017-error-boundaries.md`.
...
|--------|------|-----------|---------------------------|
| `POST` | `/wp-json/workflowy/v1/_internal/search-index/rebuild` | `RebuildSearchIndex (workspace-scoped, server-scheduled)` | IdempotencyKey (per workspace per day) |
...
- **Tables touched:** search_index, nodes
- **Error boundary on failure:** `SearchBoundary`
