...
|--------|------|-----------|---------------------------|
| `POST` | `/wp-json/workflowy/v1/sync/replay` | `ReplayQueue (FIFO; on reconnect)` | IdempotencyKey (per op) |

### SSE Frames Emitted (read-signal only, ADR-0025)

`QueueReplayProgress`, `QueueDrained` on `/stream/page/{id}` and/or `/stream/user/{id}`. SSE MUST NOT enqueue to the FIFO.

### Storage

- **Tables touched:** queue_ledger (IDB mirror) → app.nodes on flush
