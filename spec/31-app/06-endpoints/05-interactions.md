...
**Read:** App DB (per-workspace; one SQLite file per workspace) — `Items` (drag/drop target validation).
**Write:** App DB (per-workspace; one SQLite file per workspace) — `Items.ParentId` + `Items.SortOrder` (drag), `Items.Content` (inline edit).
**SSE delivery:** `/stream/page/{id}` per ADR-0025.
