...
**Read:** App DB (per-workspace; one SQLite file per workspace) — `MirrorGroup`, `MirrorMember`, `Items`.
**Write:** App DB (per-workspace; one SQLite file per workspace) — atomic peer-group mutations (add/remove member; auto-dissolve singleton via trigger `04-app-triggers.sql`).
**Root DB:** untouched.
