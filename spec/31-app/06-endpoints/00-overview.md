...
| E5 | Every authenticated route MUST call `Auth::hasRole($userId, $role)` server-side (gate `G-24-PRIVILEGE-MUTATION-GATED`). Client-trusted role checks are forbidden. | [`../01-features/15-roles-and-permissions.md`](../01-features/15-roles-and-permissions.md) |
