...
**Read:** App DB (per-workspace; one SQLite file per workspace) — `Items WHERE DeletedAt IS NOT NULL`.
**Write:** App DB (per-workspace; one SQLite file per workspace) — `Items.DeletedAt` (soft-delete) / NULL (restore) / row purge (permanent delete).
**Root DB:** untouched (trash is workspace-local).
