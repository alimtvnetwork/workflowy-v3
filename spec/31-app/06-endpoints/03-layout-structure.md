...
- **Favorites canonical placement (resolves ambiguity-triage #17, ruling 2026-04-27; ratified by [ADR-0001](../../00-adrs/0001-singular-ddl-vs-plural-prose.md) on 2026-04-28):** Favorites is the **`Item.IsFavorite` column** on the singular `Item` table — **there is no `Favorite` / `Favorites` table**. **No shell endpoint owns Favorites.** Reads piggyback on `EP-ITEMS-LIST` (`?includeFavorites=1`); writes piggyback on `EP-ITEMS-UPDATE` (`{ "IsFavorite": true|false }`). Adding an `EP-FAVORITES-*` family or promoting Favorites to its own table **MUST** first supersede ADR-0001 with a new ADR.
