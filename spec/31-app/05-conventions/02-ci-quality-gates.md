...
| G-31 | `31-check-workflow-xref-reciprocity.mjs` | Hard *(v2.0.0 implemented 2026-04-27; promoted from F25 prototype `/tmp/audit_xrefs.mjs`)* | **Multi-scope reciprocity check** with per-scope `mode: "error" \| "warn"`. **G-31.1 (workflows, ERROR)**: every cross-flow link in `## Related` of `02-workflows/NN-*-flow.md` MUST be reciprocated. **G-31.2 (features, WARN, v2.0.0)**: same invariant on `01-features/NN[a-z]?-*.md` (excl. overviews/personas/AT-registry); accepts `## Related`/`## Cross-References`/`## See also`. **G-31.3 (endpoints, WARN, v2.0.0)**: same on `06-endpoints/NN[a-z]?-*.md` (excl. overview/AT-matrix). **G-31.4 (db-diagram, WARN, v2.0.0)**: same on `07-db-diagram/NN-*.md` top-level (skips `sql/` subfolder). WARN scopes report drift in stdout but do not fail CI — staged-rollout pattern (mirrors F24/F27/F28 G-30.2). Promotion to ERROR is per-scope: drain queue → flip `mode: "warn"` → `mode: "error"` in runner's `SCOPES` registry. Each scope has its own exemption Set (4 separate Sets, all empty at v2.0.0). Catches the F25 drift class plus same-class drift across 3 additional folders (44 pre-existing asymmetries surfaced at intro: 30 features + 8 endpoints + 6 db-diagram). See [`24-g31-workflow-xref-reciprocity-gate.md`](./24-g31-workflow-xref-reciprocity-gate.md) |
