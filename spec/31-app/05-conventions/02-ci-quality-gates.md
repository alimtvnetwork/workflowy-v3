...
| G-31 | `31-check-workflow-xref-reciprocity.mjs` | Hard *(v2.0.0 implemented 2026-04-27; G-31.3 promoted WARN→ERROR in v2.1.0; G-31.4 promoted WARN→ERROR in v2.2.0 — same day; promoted from F25 prototype `/tmp/audit_xrefs.mjs`)* | **Multi-scope reciprocity check** with per-scope `mode: "error" \| "warn"`. **G-31.1 (workflows, ERROR)**: every cross-flow link in `## Related` of `02-workflows/NN-*-flow.md` MUST be reciprocated. **G-31.2 (features, WARN, v2.0.0)**: same invariant on `01-features/NN[a-z]?-*.md` (excl. overviews/personas/AT-registry); accepts `## Related`/`## Cross-References`/`## See also`. **G-31.3 (endpoints, ERROR, v2.1.0)**: same on `06-endpoints/NN[a-z]?-*.md` (excl. overview/AT-matrix); drained 8→0. **G-31.4 (db-diagram, ERROR, v2.2.0)**: same on `07-db-diagram/NN-*.md` top-level (skips `sql/` subfolder); drained 6→0. WARN scopes report drift in stdout but do not fail CI — staged-rollout pattern (mirrors F24/F27/F28 G-30.2). Promotion to ERROR is per-scope: drain queue → flip `mode: "warn"` → `mode: "error"` in runner's `SCOPES` registry. Each scope has its own exemption Set (4 separate Sets, all empty). Catches the F25 drift class plus same-class drift across 3 additional folders (current state: 30 features WARN; 0 ERROR-scope asymmetries across workflows + endpoints + db-diagram). See [`24-g31-workflow-xref-reciprocity-gate.md`](./24-g31-workflow-xref-reciprocity-gate.md) |
