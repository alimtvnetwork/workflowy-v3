...
| G-31 | `31-check-workflow-xref-reciprocity.mjs` | Hard *(implemented 2026-04-27; promoted from F25 prototype `/tmp/audit_xrefs.mjs`)* | Every cross-flow link in a `## Related` section of `spec/31-app/02-workflows/NN-*-flow.md` MUST be reciprocated by a back-link in the target's own `## Related` section. Catches the F25 drift class (workflow files growing forward-references without back-links). Allow-list `ASYMMETRIC_BY_DESIGN` excludes intentional one-way references with rationale. See [`24-g31-workflow-xref-reciprocity-gate.md`](./24-g31-workflow-xref-reciprocity-gate.md) |
| G-32 | `32-check-ddl-unique-coverage.mjs` | Hard *(v2.0.0 implemented 2026-04-27; promoted from F26 prototype `/tmp/audit_unique.mjs`)* | **Two-direction check.** **G-32.1 (forward)**: every `UNIQUE` declaration in `01-root-schema.sql` and `02-app-schema.sql` MUST be documented in `06-indexes.md` — either as an explicit `IdxXxx` row or as a `sqlite_autoindex_Table` row in §Implicit Indexes. **G-32.2 (reverse, v2.0.0)**: every `Idx*` / `sqlite_autoindex_*` identifier mentioned in backticks in `06-indexes.md` MUST resolve to (a) an explicit `CREATE [UNIQUE] INDEX` in any of the 3 SQL files, (b) a UNIQUE-implied autoindex on a real table, or (c) a documented alias in `sql/00-overview.md` §Index-name aliases. Catches the F26 drift classes: fabricated soft-delete columns, undocumented explicit CREATE UNIQUE INDEX, misclassified UNIQUE-side-effect indexes (forward), and **fabricated index names with no DDL backing** (reverse). Allow-lists `COVERAGE_EXEMPT` (forward) and `REVERSE_EXEMPT` (reverse) suppress intentional cases with rationale. See [`25-g32-ddl-unique-coverage-gate.md`](./25-g32-ddl-unique-coverage-gate.md) |
