...
| G-30 | `30-check-at-citation-validity.mjs` | Hard *(implemented 2026-04-27; scope extended same day; G-30.2 advisory added 2026-04-27; advisory promoted to default-on 2026-04-27; G-30.2 promoted WARN→ERROR v1.5.0 same day)* | Every `AT-*` ID cited under three consumer scopes — `spec/31-app/06-endpoints/**/*.md`, `spec/31-app/02-workflows/**/*.md`, and `spec/31-app/07-db-diagram/04-feature-slices.md` — MUST be declared somewhere under `spec/31-app/**` — either as a closed table-row ID (`\| \`AT-FOO-01\` \|` or `\| AT-FOO-01 \|`), a backticked range (`` `AT-FOO-01..05` ``), or a backticked open-prefix placeholder (`` `AT-FOO-NN` ``). Catches typos and invented prefixes (e.g. `AT-MGP-*`). **G-30.2 (v1.5.0)** is now an **ERROR** sub-check enforcing that no open-prefix declaration is redundant (citations 100% covered by closed declarations OR zero usage). Drained 5→0 in v1.3.0 (F27), default-on WARN in v1.4.0 (F28), promoted to ERROR in v1.5.0 (F-future-G30-A) — staged rollout pattern matching G-31.2/3/4. Negative-tested. Emergency bypass: `G30_REDUNDANT_ENFORCE=0` env var; `--warn-redundant-only` CLI flag restores WARN-only for one-off audits. Allow-list stable at 41 entries across 3 intent-categories. See [`23-g30-at-citation-validity-gate.md`](./23-g30-at-citation-validity-gate.md) |
