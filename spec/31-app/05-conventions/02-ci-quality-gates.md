...
| G-31 | `31-check-workflow-xref-reciprocity.mjs` | Hard *(v2.0.0 implemented 2026-04-27; G-31.3 promoted v2.1.0; G-31.4 promoted v2.2.0; G-31.2 promoted v2.3.0 — all same day; rollout complete)* | **Multi-scope reciprocity check** — all 4 sub-checks now ERROR-mode at 0 asymmetries. **G-31.1 (workflows, ERROR, v1.0.0)**: cross-flow links in `## Related` of `02-workflows/NN-*-flow.md` MUST be reciprocated. **G-31.2 (features, ERROR, v2.3.0)**: same on `01-features/NN[a-z]?-*.md` (excl. overviews/personas/AT-registry); accepts `## Related`/`## Cross-References`/`## See also`; drained 30→0 programmatically. **G-31.3 (endpoints, ERROR, v2.1.0)**: same on `06-endpoints/NN[a-z]?-*.md` (excl. overview/AT-matrix); drained 8→0. **G-31.4 (db-diagram, ERROR, v2.2.0)**: same on `07-db-diagram/NN-*.md` top-level (skips `sql/` subfolder); drained 6→0. WARN-then-ERROR staged rollout (mirroring F24/F27/F28 G-30.2) is now complete — no scope remains in WARN mode. Each scope retains its own exemption Set (4 separate Sets, all empty). Catches the F25 drift class plus same-class drift across 3 additional folders. See [`24-g31-workflow-xref-reciprocity-gate.md`](./24-g31-workflow-xref-reciprocity-gate.md) |
