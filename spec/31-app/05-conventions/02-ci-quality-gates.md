...
| G-30 | `30-check-at-citation-validity.mjs` | Hard *(implemented 2026-04-27; scope extended same day; G-30.2 advisory added 2026-04-27)* | Every `AT-*` ID cited under three consumer scopes — `spec/31-app/06-endpoints/**/*.md`, `spec/31-app/02-workflows/**/*.md`, and `spec/31-app/07-db-diagram/04-feature-slices.md` — MUST be declared somewhere under `spec/31-app/**` — either as a closed table-row ID (`\| \`AT-FOO-01\` \|` or `\| AT-FOO-01 \|`), a backticked range (`` `AT-FOO-01..05` ``), or a backticked open-prefix placeholder (`` `AT-FOO-NN` ``). Catches typos and invented prefixes (e.g. `AT-MGP-*`). **G-30.2 (v1.2.0)** adds an opt-in `--warn-redundant` advisory listing open prefixes whose citations are 100% closed-covered or unused — WARN-only, never fails CI. See [`23-g30-at-citation-validity-gate.md`](./23-g30-at-citation-validity-gate.md) |
