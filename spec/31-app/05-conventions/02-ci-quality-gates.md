...
| G-32 | `32-check-ddl-unique-coverage.mjs` | Hard *(v4.0.0 implemented 2026-04-27; promoted from F26 prototype `/tmp/audit_unique.mjs`)* | **Four-direction check.** **G-32.1 (forward, UNIQUE-only)**: every `UNIQUE` declaration in `01-root-schema.sql` and `02-app-schema.sql` MUST be documented in `06-indexes.md`. **G-32.2 (reverse)**: every `Idx*` / `sqlite_autoindex_*` identifier in backticks in `06-indexes.md` MUST resolve to an explicit `CREATE [UNIQUE] INDEX`, a UNIQUE-implied autoindex, or a documented alias. **G-32.3 (forward, all CREATE INDEX, v3.0.0)**: every `CREATE INDEX` (UNIQUE OR plain) across all 3 SQL files MUST appear in `06-indexes.md` by its DDL name OR its prose-alias name. **G-32.4 (meta, v4.0.0)**: every entry in `COVERAGE_EXEMPT` / `REVERSE_EXEMPT` / `NONUNIQUE_EXEMPT` MUST carry a rationale comment (trailing inline `//` OR a `//` line directly above; no blank-line gap). Catches the F26 drift classes plus **fabricated index names** (G-32.2), **undocumented non-UNIQUE indexes** (G-32.3 — caught `IdxMirrorGroup_CanonicalItemId` on first run), and **rationale-free allow-list entries** (G-32.4 — caught 4 `REVERSE_EXEMPT` siblings sharing a banner-style category comment on first run). Predicate / column drift is **out of scope** (name-presence only). See [`25-g32-ddl-unique-coverage-gate.md`](./25-g32-ddl-unique-coverage-gate.md) |
