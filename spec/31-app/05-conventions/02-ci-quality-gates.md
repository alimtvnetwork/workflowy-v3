...
| G-30 | `30-check-at-citation-validity.mjs` | Hard *(implemented 2026-04-27; scope extended same day; G-30.2 advisory added 2026-04-27; advisory promoted to default-on 2026-04-27)* | Every `AT-*` ID cited under three consumer scopes — `spec/31-app/06-endpoints/**/*.md`, `spec/31-app/02-workflows/**/*.md`, and `spec/31-app/07-db-diagram/04-feature-slices.md` — MUST be declared somewhere under `spec/31-app/**` — either as a closed table-row ID (`\| \`AT-FOO-01\` \|` or `\| AT-FOO-01 \|`), a backticked range (`` `AT-FOO-01..05` ``), or a backticked open-prefix placeholder (`` `AT-FOO-NN` ``). Catches typos and invented prefixes (e.g. `AT-MGP-*`). **G-30.2 (v1.4.0)** is a default-on advisory listing open prefixes whose citations are 100% closed-covered or unused — WARN-only, never fails CI. Opt-out via `--no-warn-redundant` or `G30_WARN_REDUNDANT=0`. See [`23-g30-at-citation-validity-gate.md`](./23-g30-at-citation-validity-gate.md) |
| G-31 | `31-check-workflow-xref-reciprocity.mjs` | Hard *(v2.0.0 implemented 2026-04-27; G-31.3 promoted v2.1.0; G-31.4 promoted v2.2.0; G-31.2 promoted v2.3.0; G-31.5 meta added v2.4.0; G-31.6 island advisory added v2.5.0 — all same day)* | **Multi-scope reciprocity check + meta + island advisory** — 5 ERROR sub-checks at 0 violations + 1 WARN sub-check surfacing 14 islands. **G-31.1 (workflows, ERROR, v1.0.0)**: cross-flow links in `## Related` of `02-workflows/NN-*-flow.md` MUST be reciprocated. **G-31.2 (features, ERROR, v2.3.0)**: same on `01-features/NN[a-z]?-*.md` (excl. overviews/personas/AT-registry); accepts `## Related`/`## Cross-References`/`## See also`; drained 30→0 programmatically. **G-31.3 (endpoints, ERROR, v2.1.0)**: same on `06-endpoints/NN[a-z]?-*.md` (excl. overview/AT-matrix); drained 8→0. **G-31.4 (db-diagram, ERROR, v2.2.0)**: same on `07-db-diagram/NN-*.md` top-level (skips `sql/` subfolder); drained 6→0. **G-31.5 (meta, ERROR, v2.4.0)**: every entry in each per-scope exemption Set (8 Sets total: 4 reciprocity + 4 island) MUST carry a rationale comment (trailing inline `//` OR a `//` line directly above; no blank-line gap; sample template lines skipped). Algorithm ported verbatim from G-32.4. **G-31.6 (islands, WARN, v2.5.0)**: per-scope advisory flagging files with **zero outgoing AND zero incoming** cross-sibling references; opt-out via `*_ISLAND_EXEMPT` Set with rationale (G-31.5-enforced). Initial state: workflows 0, features 5, endpoints 9, db-diagram 0 (14 total). WARN-only — does not fail CI; cleanup deferred to F-future-G31e. WARN-then-ERROR staged rollout (mirroring F24/F27/F28 G-30.2) is complete for the 4 reciprocity scopes. Catches the F25 drift class plus same-class drift across 3 additional folders plus rationale-free exemptions plus documentation islands. See [`24-g31-workflow-xref-reciprocity-gate.md`](./24-g31-workflow-xref-reciprocity-gate.md) |
