...
| G-30 | `30-check-at-citation-validity.mjs` | Hard *(implemented 2026-04-27)* | Every `AT-*` ID cited in `spec/31-app/06-endpoints/**/*.md` MUST be declared somewhere under `spec/31-app/**` — either as a closed table-row ID (`\| \`AT-FOO-01\` \|` or `\| AT-FOO-01 \|`), a backticked range (`` `AT-FOO-01..05` ``), or a backticked open-prefix placeholder (`` `AT-FOO-NN` ``). Catches typos and invented prefixes (e.g. `AT-MGP-*`). See [`23-g30-at-citation-validity-gate.md`](./23-g30-at-citation-validity-gate.md) |
