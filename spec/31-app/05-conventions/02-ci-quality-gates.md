...
| G-33 | `33-check-cross-runner-duplicates.mjs` | Hard *(v1.0.0 implemented 2026-04-27 as F-future-G33)* | **Cross-runner allow-list duplicate detector.** Companion to the G-30.3 / G-31.5 / G-32.4 meta sub-checks (which see only their own runner) and the G-35 reporter (which only inventories). Scans all 3 meta-checked runners; for any string value appearing in 2+ allow-lists, classifies by gate-spread: same-gate memberships are INFO (orthogonal scopes within one gate, suppressed unless `--info`); cross-gate memberships are ERROR (the same artifact is being exempted from 2+ unrelated invariants — rationale will fragment and drift). Active opt-out via `CROSS_GATE_EXEMPT` Set keyed on `"<value>::<sorted-gates>"` composites with rationale comments. Initial run: 0 cross-gate duplicates across 17 lists / 65 entries. Negative-tested via `"AT-FOO-"` injection into G-32's `COVERAGE_EXEMPT` (correctly exits 1; restores green). Parser duplicated verbatim from G-35 by design (cross-runner imports between hygiene scripts would create the kind of coupling these gates surface). See [`27-g33-cross-runner-duplicate-gate.md`](./27-g33-cross-runner-duplicate-gate.md) |
| G-34 | `34-check-allow-list-age.mjs` | Hard *(v1.0.0 implemented 2026-04-27 as F-future-G34)* | **Allow-list entry age tracker.** Adds time pressure to exemption debt: scans the same 17 allow-lists across G-30/G-31/G-32 (parser duplicated from G-33/G-35 by design — see G-33 row), runs `git blame -L <line>,<line> --porcelain` per entry to obtain the author-time of the line that introduced it, and buckets by age: INFO < 180d (fresh, suppressed unless `--info`), WARN ≥ 180d (stale, review recommended), ERROR ≥ 365d (rotted, must justify or remove). Per-entry opt-out via `AGE_EXEMPT` Set keyed on `"<list-name>::<value>"` composites caps severity at INFO. **Sub-check G-34.2 (meta):** every `AGE_EXEMPT` line MUST carry a rationale comment (inline or directly above) — runner self-introspects via `__filename`. Initial run: 0 stale / 0 rotted across 65 entries (allow-lists are 1–4 days old; gate becomes meaningful as the spec ages). Bucketing logic verified standalone (400d → ERROR, exit 1); G-34.2 verified by injecting `"FAKE::no-rationale-here"` (correctly exits 1; restored). See [`28-g34-allow-list-age-gate.md`](./28-g34-allow-list-age-gate.md) |
| G-35 | `35-allow-list-inventory.mjs` | Hard *(v1.0.0 implemented 2026-04-27 as F-future-G30-C)* | **Companion reporter to G-30.3 / G-31.5 / G-32.4 meta sub-checks.** Scans all 3 meta-checked runners (G-30, G-31, G-32) and emits a unified markdown inventory at `spec/31-app/05-conventions/26-allow-list-inventory.md` listing every exemption Set with: parent gate, sub-check, entry count, per-list rationale-coverage stat, line-anchored source link, and per-entry rationale snippet. CI invokes with `--check` flag (exit 1 on drift); operator regenerates with bare invocation. The intent is **operator visibility**: when meta gates report "0 unrationaled", this report surfaces WHAT is being exempted so allow-list bloat does not hide behind a green ✅. Initial run: 17 lists, 65 entries, 0 unrationaled. Drift detection normalises the `updated:` date so daily CI runs don't trip on date-only changes. See report at [`./26-allow-list-inventory.md`](./26-allow-list-inventory.md). |
| G-36 | `36-check-cross-scope-islands.mjs` | Hard *(v1.0.0 implemented 2026-04-27 as F-future-G31h)* | **Cross-scope true-orphan detector.** Strict-superset of G-31.6 (per-scope islands). Builds a single global inbound-reference index across `spec/**/*.md` + `src/**/*.{ts,tsx,css}` + `.lovable/**/*.md` (~1479 files), then for each of the 58 candidate files in the 4 G-31 scopes (workflows / features / endpoints / db-diagram, same `filenameRx` + `excludeRx`), counts inbound bare-filename matches excluding self + same-directory siblings. **G-36.1 (true-orphan scan, WARN advisory):** files with zero global inbound count flagged as TRUE ORPHAN; per-file opt-out via `TRUE_ORPHAN_EXEMPT` Set keyed on bare filename. **G-36.2 (meta, ERROR):** every `TRUE_ORPHAN_EXEMPT` line MUST carry rationale comment (recursive G-30.3 pattern; runner self-introspects via `__filename`). Bare-filename literal matching chosen over MD-link parsing because inbound refs appear in many shapes (`mem://`, code comments, import paths, prose); false positives tolerable for advisory severity. Initial run: 0 true orphans across 58 candidates — confirms all 14 G-31.6 per-scope islands ARE linked from outside their own scope (the WARN-with-zero-output state validates the design). G-36.2 verified via `"FAKE-no-rationale.md"` injection (exit 1; restored). Renumbered from F-future-G31h to standalone G-36 because G-31 cluster is bounded at 7 sub-checks and corpus-walk machinery deserves its own home. See [`29-g36-cross-scope-island-gate.md`](./29-g36-cross-scope-island-gate.md). |
| G-37 | `37-check-stale-relative-links.mjs` | Hard *(v1.0.0 implemented 2026-04-27 as F-future-G37)* | **Stale relative-link detector.** Companion to G-03: when G-03 reports a broken `[label](./x.md)` in `spec/**/*.md`, G-37 re-classifies by whether `basename(href)` exists elsewhere in the repo. **G-37.1 (WARN advisory):** STALE-RENAME (1 candidate elsewhere → suggested replacement path), STALE-AMBIGUOUS (≥2 candidates → operator must disambiguate), STALE-MISSING (0 candidates; suppressed unless `--verbose` because that's G-03's territory). Per-link opt-out via `STALE_LINK_EXEMPT` Set keyed on `"<source-rel>::<href>"` composites. **G-37.2 (ERROR meta):** every `STALE_LINK_EXEMPT` line MUST carry rationale comment (algorithm ported verbatim from G-30.3/G-31.5/G-32.4/G-34.2/G-36.2). Bare-basename matching chosen over path-suffix or git-history models for zero-git-dependency + filename-uniqueness-by-G-01 + WARN-only severity tolerance (see ambiguity #42). Initial run: 0 stale-rename, 0 stale-ambiguous, 6 stale-missing (G-03 backlog). G-37.2 negative-tested. See [`30-g37-stale-relative-link-gate.md`](./30-g37-stale-relative-link-gate.md). |

> **Note** — `G-14` is reserved (see `14-split-oversized-files.mjs` — operator-only, not part of the runner). `G-17` and `G-18` are reserved for the hotkey-drift check (A-19 follow-up) and the cycle-algorithm SQL drift check (A-38 follow-up) respectively, both already promised in their parent specs. **`G-19`–`G-28` algorithm SSOTs exist** in this folder (`04-`, `06-`, `07-`, `15-`, `16-`, `17-`, `18-`, `19-`, `20-`, `21-` respectively); pending runner implementation. **`G-29` through `G-37` are fully implemented** as `scripts/spec-hygiene/29-check-endpoint-matrix-coverage.mjs`, `30-check-at-citation-validity.mjs`, `31-check-workflow-xref-reciprocity.mjs`, `32-check-ddl-unique-coverage.mjs`, `33-check-cross-runner-duplicates.mjs`, `34-check-allow-list-age.mjs`, `35-allow-list-inventory.mjs`, `36-check-cross-scope-islands.mjs`, and `37-check-stale-relative-links.mjs`. **Stale-link / orphan cluster complete** — no further G-3X reservations pending.
