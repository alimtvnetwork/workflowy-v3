...
| G-32 | `32-check-ddl-unique-coverage.mjs` | Hard *(implemented 2026-04-27; promoted from F26 prototype `/tmp/audit_unique.mjs`)* | Every `UNIQUE` declaration in `spec/31-app/07-db-diagram/sql/01-root-schema.sql` and `02-app-schema.sql` MUST be documented in `spec/31-app/07-db-diagram/06-indexes.md` — either as an explicit `IdxXxx` row (Required-Indexes table) or as an implicit `sqlite_autoindex_Table` row (§Implicit Indexes). Catches the F26 drift class: fabricated soft-delete columns referenced in docs but absent from DDL, explicit `CREATE UNIQUE INDEX` rows added to DDL but never documented, misclassified UNIQUE-side-effect indexes. Allow-list `COVERAGE_EXEMPT` excludes intentional doc-elsewhere cases with rationale. See [`25-g32-ddl-unique-coverage-gate.md`](./25-g32-ddl-unique-coverage-gate.md) |
