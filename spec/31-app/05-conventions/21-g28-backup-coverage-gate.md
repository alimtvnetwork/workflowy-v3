...
5. **Tarball builder dynamically computes file list from `glob()`** — axis 4 cannot statically inspect glob expansions. **Resolution**: glob patterns MUST (gate G-BACKUP-NO-SENSITIVE-IN-TARBALL) exclude `wp-config*` and `*secret*`/`*password*`/`*auth_key*` via explicit `array_filter`; the filter call MUST (gate G-BACKUP-NO-SENSITIVE-IN-TARBALL) appear within 10 lines of the glob and reference all four exclusions. Future enhancement: parse glob patterns; for now, reviewers verify.
...
8. **Drill scheduler uses class constant defined elsewhere** — `const CADENCE = self::DEFAULT_CADENCE;`. Axis 6 misses the indirection. **Resolution**: cadence MUST (gate G-BACKUP-DRILL-CADENCE-90D) be defined inline in `DrillScheduler.php` as a literal `90` / `90 * DAY_IN_SECONDS` / `7776000`; class-constant chains forbidden by review.
