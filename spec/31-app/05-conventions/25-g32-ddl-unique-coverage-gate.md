...
| G-32.5   | parity    | For every `CREATE [UNIQUE] INDEX`, does its doc row in `06-indexes.md` mention (a) every column from the DDL `(cols)` list (case-sensitive identifier match in backticked content, with column-alias resolution via `sql/00-overview.md` §Naming Bridge), (b) the literal `UNIQUE` token if the DDL declares `UNIQUE`, and (c) the word `partial` AND the `WHERE` predicate verbatim if the DDL has a `WHERE` clause? | v5.0.0   |
...
| 4.0.0 | 2026-04-27 | F-future-G32c — added **G-32.4 meta sub-check** enforcing rationale comments on all 3 allow-lists (`COVERAGE_EXEMPT` / `REVERSE_EXEMPT` / `NONUNIQUE_EXEMPT`). New `findUnrationaledEntries()` parses the runner's own source via `readOrFail(__self_path)`, walks each `new Set([…])` block, accepts a rationale as either (a) a trailing inline `// …` on the entry's line OR (b) a `// …` comment line immediately above (no blank-line gap; pure separators like `// ----` are skipped); sample-template `// "Foo:Bar"` lines are not counted as active entries. **Caught real drift on first run**: 4 `REVERSE_EXEMPT` entries (`IdxItem_CreatedAt`, `IdxComment_AuthorUserId`, `IdxMirror_SourceItemId`, `IdxMirror_MirrorItemId`) shared a banner-style category comment with siblings above them but lacked individual rationales — fixed by adding inline `// §"Indexes NOT created" — …` and `// dropped by M-117 (legacy Mirror table)` rationales per entry. Negative-tested by injecting `"IdxFabricated_NoRationale",` (correctly exits 1 with source line). Final exit = OR(G-32.1, G-32.2, G-32.3, G-32.4). Removed F-future-G32c from future-ladder; only F-future-G32d (predicate parity, needs SQL parser) remains. |
| 5.0.0 | 2026-04-27 | F-future-G32d — added **G-32.5 column / predicate parity sub-check**. New helpers: `parseCreateIndexBlocks()` (multi-line regex `CREATE [UNIQUE] INDEX … ON Table (cols) [WHERE …];` — captures 29 blocks across 3 SQL files, preserves column qualifiers like `DESC`, strips trailing `;` from WHERE), `findDocRow()` (matches a single `\|`-prefixed table row in `06-indexes.md` containing the DDL name OR its prose alias inside backticks), `bareColName()` (strips `ASC`/`DESC` for identifier matching), `collectColumnAliasMap()` (DDL→prose column-name map built from `sql/00-overview.md` §Naming Bridge `Table.Col` rows; only differing pairs registered, e.g. `MirrorGroupId`↔`MirrorPeerGroupId`), `checkParity()` enforces 3 aspects per block: (a) **columns** — every column reference must appear inside backticks on the row, with alias fallback (e.g. row says `(MirrorPeerGroupId)` while DDL says `(MirrorGroupId)`); (b) **unique** — DDL `UNIQUE` requires the literal `UNIQUE` token in the row; (c) **predicate** — DDL `WHERE …` requires both the word `partial` AND the verbatim normalised predicate substring on the row. New `PARITY_EXEMPT` allow-list (empty as of v5.0.0; format `${name}:${aspect}` for fine-grained per-aspect suppression rather than whole-row). **Caught real drift on first run** — `IdxMirrorMember_MirrorGroupId` doc row referenced `MirrorPeerGroupId` (prose alias) while DDL says `MirrorGroupId` — resolved by adding column-alias resolution rather than allow-listing the divergence (the alias was already documented in §Naming Bridge but unused by the parity check). Negative-tested by sed-stripping `partial \`WHERE DueDate IS NOT NULL\`` from the `IdxItem_DueDate` row (correctly reported `[predicate] missing word \`partial\`; missing predicate "WHERE DueDate IS NOT NULL"`; restored). Final exit = OR(G-32.1, G-32.2, G-32.3, G-32.4, G-32.5). Current state ✅ 29 blocks parsed, 0 parity violations. Closes the F-future-G32d ladder slot; remaining future ideas (per-aspect rationale enforcement, multi-row tolerance) deferred without a slot. |
