...
| 2 | `G-13-FIXTURE-AS-SPEC-SHAPE` | [`spec/13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md`](./13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md) | [`scripts-as-spec/_TEST-CORPUS/PHASE-4-PASS-baseline.md`](./13-cicd-pipeline-workflows/scripts-as-spec/_TEST-CORPUS/PHASE-4-PASS-baseline.md) | 2026-04-28 | **Scope: test-corpus only.** The cited gate's registry row legitimately points at the production fixture; the test-corpus baseline cites the same gate purely to exercise the positive Phase-4 path. Without this exemption, the test-corpus baseline could not pass — defeating its purpose as a frozen positive case. The production scan never sees this file (non-recursive `glob("*.md")`), so the production symmetry guarantee is unaffected. |
