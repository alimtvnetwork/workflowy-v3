...
| `G-13-FIXTURE-AS-SPEC-SHAPE` | **DOC-NORM** | [`spec/13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md`](./13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md) | Phase 3: every file in `scripts-as-spec/` (except `README.md`) MUST contain the 6 required H2 sections; the Algorithm section MUST contain a fenced code block tagged with an allowed language (`python`/`bash`/`sh`/`js`/`ts`); AND the file's banner blockquote MUST cite at least one gate ID that resolves to a registered row in this file. Closes the fixture→registry traceability loop. |
