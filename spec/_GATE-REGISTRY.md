...
| `G-09-OVERVIEW-H1-MATCHES-FOLDER-INDEX` | **CI** | [`spec/01-spec-authoring-guide/97-acceptance-criteria.md`](./01-spec-authoring-guide/97-acceptance-criteria.md) | Conditional: *if* a `spec/[0-9][0-9]*/00-overview.md` H1 matches `^# (\d{2,3})\b`, the captured prefix MUST equal the parent folder's leading numeric prefix. Closes the audit-issue-#6 regression class (`spec/09-code-block-system/00-overview.md` previously carried `08`). Sub-overview `**/00-overview.md` files matched against immediate parent. **WARN-only conditional initial mode** — H1s without a leading numeric prefix are permitted (24 of 25 top-level overviews lack one today). Promotion path: Phase 2 H1-prefix sweep (audit ledger TBD: `.lovable/memory/audit/at-h1-prefix-sweep.md`) authors the missing 24 prefixes; Phase 3 flips to hard-fail mandatory ("H1 MUST start with folder prefix"). Per-ADR files in `spec/00-adrs/` (`# ADR-NNNN — Title`) exempt — only that folder's own `00-overview.md` is in scope. |
### Domain-API
