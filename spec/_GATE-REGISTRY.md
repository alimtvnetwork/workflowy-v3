...
> **Version:** 1.2.9  
> **Updated:** 2026-04-29 — patch: Scoring backfill sweep + `G-00-OVERVIEW-SCORING-TABLE-COMPLETE` rule 1 promoted WARN→hard-fail. Sweep touched 20 overviews: added `Health Score` aggregate row to 17 partial tables, normalized 2 (`33-feedback-report`, `35-enforcement-rules`) from `Confidence`→`AI Confidence`, and added a `#### Current values` block to `spec/00-adrs/00-overview.md` (its rubric had no values block). 4 overviews previously flagged as missing-schema (`03`, `05`, `06`, `07`) were false positives — `| Metric | Value |` table shape missed by initial regex. **Baseline now 25/25 clean.** Rules 2–3 remain WARN-only until a value-format normalization pass. Prior: 1.2.8 (G-00-OVERVIEW-SCORING-TABLE-COMPLETE minted WARN-only), 1.2.7 (G-00-OVERVIEW-AI-CONTRACT-COMPLETE), 1.2.6 (G-13-AUDIT-RUNNER-CONTRACT), 1.2.5 (G-00-OVERVIEW-AI-CONTRACT-PRESENT), 1.2.4 (G-00-OVERVIEW-SCORING-TABLE-PRESENT + sub-overview H1 sweep), 1.2.3 (G-09 → Phase 3), 1.2.2 (G-09 minted WARN-conditional), 1.2.1 (G-01-DOD-NO-NN-PLACEHOLDER + G-01-DOD-CONDENSED-MIRRORS-OVERVIEW), 1.2.0 (G-NS-ADR-MUST-HAS-AT), 1.1.9 (G-NS-STATUS-IN-LEGEND), 1.1.8 (G-NS-NO-DEPRECATED-ALIAS), 1.1.7 (G-01-AT-ID-FORMAT-CANONICAL), 1.1.6 (G-13-LEDGER-NUMBERING-CONTIGUOUS), 1.1.5 (G-13-LEDGER-ROW-COUNT-PARITY), 1.1.4 (G-13-PLACEHOLDER-TOKEN-PARITY), 1.1.3 (§5.1 reconciliation), 1.1.2 (G-26-WIRE-OWNERID-ONLY dual tier), 1.1.1 (added the gate), 1.1.0 (20 new gates from ADR-0012 §D7, ADR-0027, ADR-0028).  

- **Total named gates:** 305 (unchanged — sweep is content backfill + tier promotion; no new gate this patch)
- **CI:** 35 (unchanged; one gate `G-26-WIRE-OWNERID-ONLY` is dual-tier: counted under both CI and TEST)
- **TEST:** 14 (unchanged)
...
| `G-00-OVERVIEW-SCORING-TABLE-COMPLETE` | **CI** | [`spec/01-spec-authoring-guide/97-acceptance-criteria.md`](./01-spec-authoring-guide/97-acceptance-criteria.md) | Mirrors `G-00-OVERVIEW-AI-CONTRACT-COMPLETE` for the Scoring side. Three rules: (1) Scoring section MUST contain three canonical row tokens — `AI Confidence`/`AI Implementability`, `Ambiguity`, `Health Score`/`Overall`/`Total`; (2) each row MUST be followed by a parseable numeric score (`\d+%`, `\d+/\d+`, or letter grade); (3) aggregate Health/Overall/Total row MUST be last. **Dirty baseline 0/25** — 19 carry AI Confidence + Ambiguity but lack the aggregate; 6 (`00-adrs`, `01-spec-authoring-guide`, `03-error-manage`, `05-split-db-architecture`, `06-seedable-config-architecture`, `07-design-system`) lack the Scoring schema entirely. **WARN-only from day 1**; promotion to hard-fail deferred until backfill sweep scoped. Sub-overviews out of scope (Scoring lives at section root only). Fenced-code-block carve-out applies. SSOT: `spec/01-spec-authoring-guide/14-scoring-metrics.md`. Audit ledger: `.lovable/memory/audit/at-overview-scoring-complete-gate.md`. Completes Layer-2 of the trio (paired with `…-AI-CONTRACT-COMPLETE`). |
### Domain-API
