...
| `G-13-FIXTURE-AS-SPEC-SHAPE` | **DOC-NORM** | [`spec/13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md`](./13-cicd-pipeline-workflows/scripts-as-spec/fixture-as-spec-shape-audit.md) | Phase 2: every file in `scripts-as-spec/` (except `README.md`) MUST contain the 6 required H2 sections (Purpose / Inputs / Outputs / Algorithm / Exemptions / Strictness-roadmap-or-Test-fixtures); the Algorithm section MUST contain a fenced code block tagged with an allowed language (`python`/`bash`/`sh`/`javascript`/`js`/`typescript`/`ts`). Frozen reference algorithm self-audits this directory. |
