...
| `G-00-ADR-CONSEQUENCES-XLINK` | **CI** | [`spec/00-adrs/97-acceptance-criteria.md`](./00-adrs/97-acceptance-criteria.md) | Enforces `AT-ADR-009`: every ADR's `## Consequences` section MUST contain ≥1 markdown link pointing into a downstream `spec/` scope (non-ADR target). Sibling ADR links (`NNNN-*.md`) do NOT count — they're horizontal, not downstream. Runner: `scripts/spec-hygiene/47-check-adr-consequences-xlink.mjs`. **WARN-only initial mode** (CI exit 0 outside strict mode); flips hard-fail when baseline allow-list is empty. Baseline 2026-04-29: 28/28 ADRs allow-listed at `spec/00-adrs/_LEDGER-G-00-ADR-CONSEQUENCES-XLINK-BASELINE.md` (90-day TTL). Each ADR enriched with a downstream xlink in Consequences removes its row from the ledger. Drained by Consequences-enrichment sweep (parent loop task). Companion to `G-NS-ADR-MUST-HAS-AT` (citation side) and `G-00-ADR-XLINK-SYMMETRY` (Related-ADRs side). |
