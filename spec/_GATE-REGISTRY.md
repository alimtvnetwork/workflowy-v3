...
> **Version:** 1.6.3  
> **Updated:** 2026-04-29 — Per-(gate, path) Phase-3 `pathGlob` enforcement extended to G-31 + G-32 (closes the trilogy at the enforcement layer): both runners now compile each ledger row's `pathGlob` to a RegExp via shared `globToRegExp()` (≤15-line logic per ADR-0007 R3), build per-(scope/category, entry) glob maps, and consult `isG31Exempt()` / `isG32Exempt()` at every check site (G-31 × 3 sites: findAsymmetries / findIslands / findHeadingDrift; G-32 × 1 site: findFabricatedIndexes with `G32_REVERSE_HOST`). Loader hard-fails on empty `pathGlob` cells (was: silently accepted). Negative tests confirm enforcement is real: corrupting G-31 features-peer rows' `pathGlob` → all 7 asymmetries re-surface; corrupting G-32 reverse rows' `pathGlob` → all 8 names re-flagged fabricated. Trilogy status (Phase-2 + Phase-3): **G-30 ✅ G-31 ✅ G-32 ✅** — 67 ledger rows path-bounded across 3 runners. Audit: `.lovable/memory/audit/at-per-gate-path-ledger-phase3-g31-g32.md`. Prior: 1.6.2 (G-32 Phase-2).
