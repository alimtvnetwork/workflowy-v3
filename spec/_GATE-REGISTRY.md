...
> **Version:** 1.6.2  
> **Updated:** 2026-04-29 — Per-(gate, path) ledger Phase-2 trilogy COMPLETE: G-32 sibling migration created `spec/31-app/05-conventions/_LEDGER-G-32-EXEMPTIONS.md` (8 rows, all G-32.2.reverse for `06-indexes.md`); `REVERSE_EXEMPT` in `32-check-ddl-unique-coverage.mjs` reduced to empty emergency-override slot; new `loadG32Exemptions()` parser + `G32_CATEGORY_TO_SET` registry mirror the G-30/G-31 sibling pattern (markdown-backtick strip, hard-fail on schema violations, module-load union). Behaviour parity verified (`reverse-exempt: 8, fabricated: 0`). G-35 inventory regenerated → **17 allow-lists, 0 entries, 0 unrationaled** (every in-source Set across all three runners is now an empty override skeleton). Trilogy totals: G-30=41 + G-31=18 + G-32=8 = **67 ledger rows** under uniform schema. Audit: `.lovable/memory/audit/at-per-gate-path-ledger-phase2-g32.md`. Prior: 1.6.1 (G-31 sibling).
