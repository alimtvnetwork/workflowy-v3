...
> **Version:** 1.3.3  
> **Updated:** 2026-04-29 — patch: authored `scripts/spec-hygiene/18-check-scoring-value-format.mjs` (Node ESM checker, 0 deps) and wired into `00-run-all.mjs` slot 18. Codifies `G-00-OVERVIEW-SCORING-VALUE-FORMAT` Rules 1–4 in runner form: walks all 25 top-level overviews, strips fenced code blocks, enumerates every `^(##|###)\s+Scoring\b` heading, validates Health Score / AI Confidence / Ambiguity row values per canonical regex/token-set, enforces single-Scoring-section invariant. Self-tested both green path (25/25 clean → exit 0) and failure path (injected `100/100` → exit 1 with `file:line` message). Registry-runner parity preserved (`G-13-AUDIT-RUNNER-CONTRACT` Rule 6: every `\d{2}-check-*.mjs` MUST appear in `checks` array). Prior: 1.3.2 (dedupe + Rule 4), 1.3.1 (VALUE-FORMAT rules 1+2+3 promoted hard-fail), 1.3.0 (VALUE-FORMAT minted WARN-only), 1.2.9 (Scoring backfill sweep + COMPLETE rule 1 promoted), 1.2.8 (G-00-OVERVIEW-SCORING-TABLE-COMPLETE minted WARN-only), 1.2.7 (G-00-OVERVIEW-AI-CONTRACT-COMPLETE), 1.2.6 (G-13-AUDIT-RUNNER-CONTRACT), 1.2.5 (G-00-OVERVIEW-AI-CONTRACT-PRESENT), 1.2.4 (G-00-OVERVIEW-SCORING-TABLE-PRESENT + sub-overview H1 sweep), 1.2.3 (G-09 → Phase 3), 1.2.2 (G-09 minted WARN-conditional), 1.2.1 (G-01-DOD-NO-NN-PLACEHOLDER + G-01-DOD-CONDENSED-MIRRORS-OVERVIEW), 1.2.0 (G-NS-ADR-MUST-HAS-AT), 1.1.9 (G-NS-STATUS-IN-LEGEND), 1.1.8 (G-NS-NO-DEPRECATED-ALIAS), 1.1.7 (G-01-AT-ID-FORMAT-CANONICAL), 1.1.6 (G-13-LEDGER-NUMBERING-CONTIGUOUS), 1.1.5 (G-13-LEDGER-ROW-COUNT-PARITY), 1.1.4 (G-13-PLACEHOLDER-TOKEN-PARITY), 1.1.3 (§5.1 reconciliation), 1.1.2 (G-26-WIRE-OWNERID-ONLY dual tier), 1.1.1 (added the gate), 1.1.0 (20 new gates from ADR-0012 §D7, ADR-0027, ADR-0028).  
