...
|---|------|-------|-------------|
| 00 | [22-quick-start.md](22-quick-start.md) | **Quick Start Guide** | Condensed onboarding — read this first. 5 starter files, critical patterns, decision matrix, common mistakes |
...
| 04 | [04-logging-and-error-handling/](04-logging-and-error-handling/00-overview.md) | Logging & Error Handling | Two-tier logging (FileLogger + ErrorLogHelper), debug mode gate, log format, stack traces, rotation, deduplication, `safeExecute()`, shutdown handler, `DateHelper` |
| 05 | [05-helpers-responses-and-integration/00-overview.md](05-helpers-responses-and-integration/00-overview.md) | Helpers, Response Envelope & Integration | Helper classes (PathHelper, BooleanHelpers, InitHelpers, HttpConfigType), `EnvelopeBuilder` fluent API, standard response format, integration checklist, Split DB concept, Database trait decomposition |
...
| 07 | [07-reference-implementations/](07-reference-implementations/00-overview.md) | Reference Implementations | Complete working examples: bootstrap, autoloader, Plugin.php, Activator/Deactivator/uninstall, EnvelopeBuilder, ResponseKeyType enum, .ai-instructions template, PluginConfigType (split into 8 topic files) |
| 08 | [08-wordpress-integration-patterns/](08-wordpress-integration-patterns/00-overview.md) | WordPress Integration Patterns | Admin pages & settings API, AJAX handlers, WP-Cron scheduled tasks, file upload handling, SQLite migrations, database seeding from JSON |
| 09 | [09-testing-patterns/](09-testing-patterns/00-overview.md) | Testing Patterns | Internal test framework, assertion helpers, database tests, validation tests, seed data verification |
| 10 | [10-deployment-patterns/](10-deployment-patterns/00-overview.md) | Deployment Patterns | Build pipeline, `.distignore`, self-update with rollback, `SelfUpdateStatusType` enum, `UpdateResolver` trait decomposition, uninstall cleanup (split into 13 files) |
| 11 | [11-frontend-and-template-patterns/00-overview.md](11-frontend-and-template-patterns/00-overview.md) | Frontend & Template Patterns (split — 11 files) | 200-line file limit, orchestrator pattern, partial templates, JS/CSS enqueuing, optional React integration, source maps, decision matrix |
| 12 | [12-design-system/](12-design-system/00-overview.md) | Design System (split) | CSS variables, typography, color tokens, shadow hierarchy, animation library, badge patterns, card patterns, button variants, form input styling |
| 13 | [13-admin-ui-patterns/](13-admin-ui-patterns/00-overview.md) | Admin UI Patterns (split) | Page layout, filter bars, table patterns with date grouping, badge system, modal anatomy, notices, empty/loading states, stats bars, toggle switches |
| 14 | [14-rest-api-conventions/00-overview.md](14-rest-api-conventions/00-overview.md) | REST API Conventions | Route naming, namespace patterns, HTTP method selection, pagination, category grouping, `endpoints.json` format, controller organisation |
| 15 | [15-settings-architecture/](15-settings-architecture/00-overview.md) | Settings Architecture (split) | `OptionNameType`/`SettingsKeyType` enums, settings groups, defaults, validation, toggle switches, conditional display, save feedback, action buttons |
| 16 | [16-error-handling-extraction/](16-error-handling-extraction/00-overview.md) | Error Handling & Diagnostics | `ErrorType` classification, bootstrap vs FileLogger capture, `safeExecute` wrapper, admin error sessions, flash banners |
...
| 19 | [19-micro-orm-and-root-db/](19-micro-orm-and-root-db/00-overview.md) | Micro-ORM & Root Database | Fluent query builder, TypedQuery with Go-style DbResult/DbResultSet/DbExecResult wrappers, FileCache with scan/store trait decomposition, cross-plugin RootDb manifest |
| 20 | [20-end-to-end-walkthrough/](20-end-to-end-walkthrough/00-overview.md) | **End-to-End Walkthrough** | Complete "Task Tracker" plugin built from scratch — 14 steps covering all 19 phases, with phase coverage matrix and final checklist |
