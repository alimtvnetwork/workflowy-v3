...
> **Cross-Reference:** [01-registry/](./01-registry/)

---

## Purpose

Automated tool that parses the master error code registry ([01-registry/](./01-registry/)) and detects overlapping or duplicate range allocations. Prevents the class of collision bugs documented in `spec/23-how-app-issues-track/16-error-code-collision-remediation.md`.

---

## Input

The validator reads `spec/03-error-manage/03-error-code-registry/01-registry/` and extracts all rows from the **Registered Project Prefixes** table (Section 1) and all **Standalone Specification Error Ranges** detail tables.
...
Registry: spec/03-error-manage/03-error-code-registry/01-registry/
Entries parsed: {N}
...
1. **Before any error code allocation:** Run validator after editing [01-registry/](./01-registry/)
...
```
spec-tools registry validate [--registry path/to/01-registry/] [--format json|text]
```

| Flag | Default | Description |
|------|---------|-------------|
| `--registry` | `spec/03-error-manage/03-error-code-registry/01-registry/` | Path to registry folder |
