...
| AT-DEBUGGINGTYPESCRIPT-10 | The eight named error boundaries enumerated in ADR-0017 D1 (`AppErrorBoundary`, `RouterErrorBoundary`, `NavbarBoundary`, `SidebarBoundary`, `ContentAreaBoundary`, `EditorBoundary`, `PanelBoundary`, `ModalBoundary`) are each mounted exactly once and isolate their subtree. Every fallback (a) renders a region-appropriate typed UI (no generic "Something went wrong" outside `AppErrorBoundary`), (b) reports to the structured logger, (c) shows `errorCode` when present, (d) offers at least one recovery action. A single root-only `<ErrorBoundary>` is a violation of `G-22-ERROR-BOUNDARIES-EXACTLY-8`. | [`08-error-boundary.md`](./08-error-boundary.md), [`spec/00-adrs/0017-eight-error-boundaries-ui-virtualization.md`](../../../../../00-adrs/0017-eight-error-boundaries-ui-virtualization.md), [`spec/03-error-manage/02-error-architecture/04-error-modal/97-acceptance-criteria.md`](../../../02-error-architecture/04-error-modal/97-acceptance-criteria.md) |
