...
| F-SCOPE-34 | LOW | 2026-04-30 (task #6-batch-31) | Resolved | Pure new-gate seeding under brand-new **Domain-USER** namespace (greenfield — pre-flight check confirmed zero prior `G-USER-*` rows): 21 prose-MUSTs in `spec/36-user-management/97-acceptance-criteria.md` bound via same-line `[gate: G-USER-…]` citations to **22 new** gates (one row carries two MUSTs: ReferralUrl-deterministic + ReferralCode-unique → split into 2 gates). 16 gates cover the curated AT-USERMANAGEMENT-01..16 table (data model: `-PK-INTEGER-AUTOINC`, `-ROLES-SEPARATE-TABLE`, `-ROLE-ENUM-CLOSED`; RBAC: `-CAPABILITY-MATRIX-SSOT`, `-HASROLE-CENTRAL`, `-HASROLE-SECURITY-DEFINER`; auth: `-AUTH-PASSKEY-SUPPORT`, `-PWHASH-ARGON2ID`, `-SESSION-COOKIE-FLAGS`, `-JWT-USERID-ONLY`; solo: `-SOLO-NO-LOGIN`, `-SOLO-TO-SYNC-LOSSLESS`; admin: `-ADMIN-DELETE-SEPARATE`, `-ADMIN-ACTION-AUDITED`; sharing+GDPR: `-ADMIN-OVERRIDE-NOTIFIED`, `-GDPR-EXPORT-DELETE`). 6 gates cover P13-backfilled story flows (`-AUTOSAVE-OPTIMISTIC`, `-PWCHANGE-REVOKES-SESSIONS`, `-RESTORE-REJECTS-CONCURRENT`, `-THEME-SSE-CROSSTAB`, `-REFERRAL-DETERMINISTIC`, `-REFERRAL-UNIQUE`). Tier mix: 16 CI + 3 TEST + 3 DOC-NORM. Distinct from Domain-RE (role-escalation lifecycle/dual-control) which governs *changes* to assignments — `Domain-USER` governs *baseline schema, helpers, hashing, sessions, admin tooling, GDPR*. Source 21 → 0 parser-visible. Corpus 390 → **369** (Δ –21 exact, the largest single-batch reduction in the F-SCOPE series; reflects density of curated AT files vs typical conventions docs). Registry bumped 1.7.21 → 1.7.22. 25th consecutive zero-parser-drift batch. | `spec/_GATE-REGISTRY.md` v1.7.22, `spec/36-user-management/97-acceptance-criteria.md`, this row |

**Open count:** 0 — **Resolved:** 27
