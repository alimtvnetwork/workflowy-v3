...
| F-SCOPE-19 | LOW | 2026-04-29 (task #6-batch-16) | Resolved | Pure content migration (no parser change): 8 prose-MUSTs in `spec/13-cicd-pipeline-workflows/scripts-as-spec/per-gate-path-ledger-schema.md` (L8 callout + L29 Inputs + L43 Schema + L59/L61/L63/L69 Validation rules §1/§2/§3/§5 + L129 worked-example output) bound to 8 new `G-13-LEDGER-*` sub-gates of the existing umbrella `G-13-LEDGER-PER-GATE-PATH` (CI): `-IMPL-PARITY`, `-RUNNER-FILTER`, `-FIVE-COLUMN-SCHEMA`, `-SCHEMA-SHAPE`, `-GATE-EXISTENCE`, `-GLOB-NON-EMPTY`, `-RATIONALE-PROSE`, `-RUNNER-CLEAN-OUTPUT`. All sub-gates DOC-NORM (umbrella retains CI tier). Pre-flight namespace check: 24 sibling `G-13-*` gates already registered, no collisions on the new `G-13-LEDGER-*` prefix. Same-line-citation rule (NEW-8 lesson) applied from the start — zero re-bind needed. Source 8 MUSTs (parser counted 7; L8 inside `>` blockquote not counted as prose) → 0 unbound. Corpus 446 → 439 (Δ –7 = exact match to parser-visible count). 10th consecutive zero-parser-drift batch. | `spec/_GATE-REGISTRY.md` v1.7.13, `spec/13-cicd-pipeline-workflows/scripts-as-spec/per-gate-path-ledger-schema.md`, this row |
| F-SCOPE-20 | MED | 2026-04-29 (task #6-batch-17) | Resolved | Content migration + namespace-collision discovery: 7 prose-MUSTs in `spec/31-app/05-conventions/21-g28-backup-coverage-gate.md` (L16 axis 2 + L17 axis 3 + L19 axis 5 + L20 axis 6 + L21 axis 7 + L249 edge case 5 + L252 edge case 8) bound to 6 new `G-BACKUP-*` sub-gates: `-CLIENT-SIDE-ENCRYPT` (promoted from "reserved" status in umbrella row to its own row), `-S3-CONFIG-HARDENED`, `-NO-SENSITIVE-IN-TARBALL`, `-RESTORE-AUDIT-REWALK`, `-DRILL-CADENCE-90D`, `-SCHEDULE-CRON-PARITY`. All CI tier. Existing `G-BACKUP-RESTORE-INTEGRITY-CHECK` reused for axis 5 first half. **Pre-flight namespace check caught the F-SCOPE-15 pattern reoccurring:** the source file's title and §Reserved-Gate-ID line still claim `G-28`, but the `G-28-*` namespace is fully owned by ADR-0028 (i18n: `-LIBRARY-IS-I18NEXT`, `-NO-HTML-IN-JSON`, `-INTL-EXPLICIT-LOCALE`, `-TYPED-KEYS`, `-MISSING-KEY-LOGGED`, `-FALLBACK-CHAIN`, `-RTL-DIR-ATTR`, `-DETECTION-ORDER`, `-LOCALE-WRITE-VIA-QUEUE`). Existing `G-BACKUP-*` family (created by F-SCOPE-15) was used; this batch effectively closes the §axis-content portion of the F-SCOPE-15-FOLLOWUP rename — only the file-title H1 and §front-matter "Reserved Gate ID" line still say "G-28" (cosmetic; not parser-visible). Severity bumped to MED because this is the SECOND file in the corpus to misuse the `G-28` prefix despite the F-SCOPE-15 ratification — suggests file-title sweep is needed (tracked as new task NEW-9). Same-line-citation rule applied from start — zero re-bind. Source 7 → 0; corpus 439 → 432 (Δ –7 exact match). 11th consecutive zero-parser-drift batch. | `spec/_GATE-REGISTRY.md` v1.7.14, `spec/31-app/05-conventions/21-g28-backup-coverage-gate.md`, this row |

**Open count:** 0 — **Resolved:** 18
