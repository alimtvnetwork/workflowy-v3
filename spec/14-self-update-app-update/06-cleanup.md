...
This command scans the binary's directory and temp directory for leftover artifacts and removes them.
...
The update process should attempt automatic cleanup after a successful update. However, some files may be locked (e.g., the handoff copy is still running). In that case:

1. **Try to clean up immediately** after the update completes.
2. **If any files are locked**, skip them silently — they will be cleaned up on the next `update-cleanup` or next update.
...
The build script (`run.ps1` / `run.sh`) should call `update-cleanup` at the end of an update run:
...
**Important**: Never delete `.old` files during deploy. They are the rollback safety net. Only delete them after the update is confirmed successful (version check passes).

---

## Temp Directory Hygiene

The system temp directory can accumulate artifacts if updates are interrupted. The cleanup command should scan for patterns:
...
Use conservative matching — only delete files that match the exact prefix pattern for your tool.
...
- [Self-Update Overview](./01-self-update-overview.md) — Cleanup as the final update step
- [Rename-First Deploy](./03-rename-first-deploy.md) — Creates the `.old` files
- [Handoff Mechanism](./05-handoff-mechanism.md) — Creates the handoff copies

---

*Cleanup — v3.1.0 — 2026-04-10*
