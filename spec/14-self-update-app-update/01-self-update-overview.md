...
**Requires**: Source repository accessible on the machine, Go toolchain installed.

**Advantage**: The user always gets a binary built for their exact platform with embedded repo path.

### Strategy 2: Binary-Based Update (Download Pre-Built)

Used when the binary was installed via an installer script or package manager (no source repo available):
...
When using source-based updates, the tool must locate the source repo. Use a multi-tier resolution strategy:
...
Always normalize versions before comparing (strip `v` prefix, trim whitespace).
...
- The update command must be **synchronous** — the user sees all output in the same terminal session.
...
- [Deploy Path Resolution](./02-deploy-path-resolution.md) — How the target directory is determined
- [Rename-First Deploy](./03-rename-first-deploy.md) — The file replacement strategy
- [Handoff Mechanism](./05-handoff-mechanism.md) — Windows self-replacement flow
- [Cleanup](./06-cleanup.md) — Post-update artifact removal
- [Updater Binary](./13-updater-binary.md) — Standalone updater architecture for binary-based updates
- [Network Requirements](./14-network-requirements.md) — HTTP client, retry, proxy configuration
- [Configuration File](./15-config-file.md) — Where repo path and deploy path defaults are stored

---

*Self-update overview — v3.1.0 — 2026-04-13*
