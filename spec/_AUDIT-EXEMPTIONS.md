...
| `spec/13-cicd-pipeline-workflows/02-go-binary-deploy/**/*.md` | dead-runtime | Entire subdir documents the standalone-Go binary deploy pipeline — explicitly forbidden by the 2026-04-25 backend decision (Core memory: "WordPress plugin (PHP 8.1+ + SQLite + REST). … standalone Node, Cloudflare D1, Go all forbidden"). Files are retained as historical reference only; thickening them would re-legitimize a forbidden runtime. The live deploy pipeline lives under `18-wp-plugin-deploy/`. | F-AUDIT-25 burndown (task #44b, 2026-04-29); cites mem://constraints/backend-runtime-deferred | 2026-04-29 |
