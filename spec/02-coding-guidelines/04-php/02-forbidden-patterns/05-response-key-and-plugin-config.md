...
Keys that are **not** in `ResponseKeyType` remain as literal strings (e.g., domain-specific keys like `'retention'`, `'orphans'`, `'stuck'`, `'settings'`, `'providers'`, `'content'`). If a key appears in 3+ files, the author MUST promote it to the `ResponseKeyType` enum (see [`../../06-ai-optimization/05-enum-naming-quick-reference.md`](../../06-ai-optimization/05-enum-naming-quick-reference.md)).
