...
> **Rule:** If a path does not have a typed accessor in `PathHelper`, create one before using it. See [PHP Enum Spec](../01-enums/00-overview.md) for full `PathEnum` and `PathHelper` listings.
