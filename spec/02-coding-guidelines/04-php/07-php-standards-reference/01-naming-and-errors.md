...
> **Baseline:** [PSR-12 PHP Naming Conventions](../03-naming-conventions/00-overview.md)  
...
Register a shutdown handler to catch fatal errors. **Delegate the type-check to `ErrorChecker`** which uses `ErrorType::FATAL_TYPES` (see [PHP Enum Spec](../01-enums/00-overview.md)):
...
> **Implementation:** `ErrorChecker` delegates to `ErrorType::FATAL_TYPES` for the constant list. Use `ErrorChecker::getTypeLabel($error['type'])` to convert any `E_*` integer to a human-readable string (e.g., `'E_ERROR'`) — this replaces all inline type-mapping arrays. See [enums.md](../01-enums/00-overview.md) for the full `ErrorChecker`, `ErrorType`, and `TYPE_LABELS` implementations.
