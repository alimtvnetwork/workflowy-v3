...
| `coding-guidelines/no-magic-strings` | `warn` | [TS Standards §3](./08-typescript-standards-reference/00-overview.md) | No raw string literals in `===`/`!==`/`switch` — use enum/constant |
...
| `coding-guidelines/blank-line-before-return` | `warn` | [Code Style §R4](../01-cross-language/04-code-style/03-blank-lines-and-spacing/01-rule-4-before-return-throw.md) | Blank line before `return`/`throw` when preceded by statements |
...
These enforce type safety rules from [TS Standards §1–§2](./08-typescript-standards-reference/00-overview.md):
...
- `eslint.config.js` (project root) — Active rule set *(lives outside spec/)*
- `eslint-plugins/coding-guidelines/index.js` — Rule implementations *(lives outside spec/)*
- [Promise Patterns](./09-promise-await-patterns.md) — Promise.all enforcement rationale
- [TS Standards Reference](./08-typescript-standards-reference/00-overview.md) — Full TypeScript rules
