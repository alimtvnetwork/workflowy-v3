...
One concrete I/O fixture per `AC-001..034` row in this folder's prose rollup. All fixtures are pure-UI / static-analysis shape (no REST envelope) per the format SSOT's "two AT shapes that opt out of the JSON rows".

---

## Theme & Variables

### `AC-001` — No hardcoded color literals in components
...
### `AC-002` — Colors via `hsl(var(--token))`
...
### `AC-003` — Changing `--primary` updates all primary surfaces
...
### `AC-004` — `:root` and `.dark` define every token
...
### `AC-005` — Theme toggle changes all surfaces/text/borders
...
### `AC-006` — WCAG AA 4.5:1 text contrast
...
### `AC-010` — H1 / H2 use gradient text
...
### `AC-011` — No skipped heading levels
...
### `AC-012` — Hover transitions ≤ 300 ms
...
### `AC-013` — No JS animation libraries

| Linter command | `rg -n "from ['\"]\\b(framer-motion\|gsap\|anime\|lottie-web)\\b" src/` |
|---|---|
| **Expected exit code** | `1` |

### `AC-014` — `prefers-reduced-motion` disables animations
...
### `AC-016` — CTA buttons use slide-text animation
...
### `AC-019` — Font-size controls 12px..32px
...
### `AC-022` — Fullscreen fills viewport with 2rem inset
...
### `AC-023` — Escape exits fullscreen
...
### `AC-032` — No new fonts introduced per page

| Linter command | `rg -nP "font-family:" src/pages \| rg -vP "Ubuntu\|Poppins\|JetBrains Mono"` |
|---|---|
| **Expected exit code** | `1` |

### `AC-033` — Interactive elements follow state language
...
### `AC-034` — Responsive at mobile/tablet/desktop
...
- [`97-acceptance-criteria.md`](./97-acceptance-criteria.md) — Prose rollup (`AC-001..034`)
